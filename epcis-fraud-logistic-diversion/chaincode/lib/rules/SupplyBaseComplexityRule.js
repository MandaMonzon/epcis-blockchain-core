...
const { ErrorCode, STATE_KEY_PREFIX, BizStep } = require('../../constants/EpcisConstants');
const { SUPPLY_ELEVATED, SUPPLY_HIGH, SUPPLY_CRITICAL } = require('../../constants/RiskScoreConfig');

// Thresholds mapped directly from Skilton et al. (2024) Table 1
...
// Only check complexity on events where the buyer is receiving goods
...
class SupplyBaseComplexityRule extends TaxonomyRule {
    ...
    async evaluate(ctx, event) {
        ...
        const key        = STATE_KEY_PREFIX.COMPLEXITY + event.bizLocation;
        const stateBytes = await ctx.stub.getState(key);
        const state      = (stateBytes && stateBytes.length > 0)
            ? JSON.parse(stateBytes.toString())
            : { suppliers: [] };

        // Add new suppliers from this event (deduplication via Set)
        const supplierSet = new Set(state.suppliers);
        for (const src of event.sourceList) {
            // GS1 CBV: sourceList uses "source" field (not "value")
            const id = String(src.source || src.value || src.type || '').trim();
    ...
    /** Persists the updated supplier set after a valid receiving event. */
    static async updateComplexityState(ctx, event) {
        ...
        state.suppliers     = Array.from(supplierSet);
        state.supplierCount = state.suppliers.length;
        state.lastUpdated   = event.eventTime;
